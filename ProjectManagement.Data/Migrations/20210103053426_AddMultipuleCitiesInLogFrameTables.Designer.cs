// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using ProjectManagement.Data;

namespace ProjectManagement.Data.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20210103053426_AddMultipuleCitiesInLogFrameTables")]
    partial class AddMultipuleCitiesInLogFrameTables
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");

                    b.HasData(
                        new
                        {
                            Id = "5A71C6C4-9488-4BCC-A680-445A34C6E721",
                            ConcurrencyStamp = "5A71C6C4-9488-4BCC-A680-445A34C6E721",
                            Name = "Admin",
                            NormalizedName = "ADMIN"
                        },
                        new
                        {
                            Id = "9E6E9812-4A93-4F28-81F3-8B52181EFA77",
                            ConcurrencyStamp = "9E6E9812-4A93-4F28-81F3-8B52181EFA77",
                            Name = "ReadOnly",
                            NormalizedName = "READONLY"
                        },
                        new
                        {
                            Id = "B6ED309F-4F39-4862-B488-B27669C202C5",
                            ConcurrencyStamp = "B6ED309F-4F39-4862-B488-B27669C202C5",
                            Name = "FullAccess",
                            NormalizedName = "FULLACCESS"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");

                    b.HasData(
                        new
                        {
                            Id = "A0456563-F978-4135-B563-97F23EA02FDA",
                            AccessFailedCount = 0,
                            ConcurrencyStamp = "A0456563-F978-4135-B563-97F23EA02FDA",
                            Email = "admin@gmail.com",
                            EmailConfirmed = true,
                            LockoutEnabled = true,
                            NormalizedEmail = "ADMIN@GMAIL.COM",
                            NormalizedUserName = "ADMIN",
                            PasswordHash = "AQAAAAEAACcQAAAAEDch3arYEB9dCAudNdsYEpVX7ryywa8f3ZIJSVUmEThAI50pLh9RyEu7NjGJccpOog==",
                            PhoneNumberConfirmed = false,
                            SecurityStamp = "",
                            TwoFactorEnabled = false,
                            UserName = "Admin"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");

                    b.HasData(
                        new
                        {
                            UserId = "A0456563-F978-4135-B563-97F23EA02FDA",
                            RoleId = "5A71C6C4-9488-4BCC-A680-445A34C6E721"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("ProjectManagement.Data.City", b =>
                {
                    b.Property<int>("CityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("CityName")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<int>("StateId")
                        .HasColumnType("int");

                    b.HasKey("CityId");

                    b.HasIndex("StateId");

                    b.ToTable("City");
                });

            modelBuilder.Entity("ProjectManagement.Data.Company", b =>
                {
                    b.Property<int>("CompanyId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<byte[]>("Logo")
                        .HasColumnType("varbinary(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("Phone")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.HasKey("CompanyId");

                    b.ToTable("Company");

                    b.HasData(
                        new
                        {
                            CompanyId = 1,
                            Name = "Company Name"
                        });
                });

            modelBuilder.Entity("ProjectManagement.Data.Country", b =>
                {
                    b.Property<int>("CountryId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("CountryName")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.HasKey("CountryId");

                    b.ToTable("Country");
                });

            modelBuilder.Entity("ProjectManagement.Data.Donor", b =>
                {
                    b.Property<int>("DonorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("Phone")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.HasKey("DonorId");

                    b.ToTable("Donor");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame", b =>
                {
                    b.Property<int>("LogFrameId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Activity")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("Outcome")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("OutcomeBaseIndicator")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("Output")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("OutputBaseIndicator")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("ProjectGoal")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.Property<string>("ResultBaseIndicator")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.HasKey("LogFrameId");

                    b.HasIndex("ProjectId")
                        .IsUnique();

                    b.ToTable("LogFrame");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame1stStepCity", b =>
                {
                    b.Property<int>("LogFrame1stStepCityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CityId")
                        .HasColumnType("int");

                    b.Property<int>("LogFrame1stStepIndicatorId")
                        .HasColumnType("int");

                    b.HasKey("LogFrame1stStepCityId");

                    b.HasIndex("CityId");

                    b.HasIndex("LogFrame1stStepIndicatorId");

                    b.ToTable("LogFrame1stStepCity");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame1stStepIndicator", b =>
                {
                    b.Property<int>("LogFrame1stStepIndicatorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal?>("AchieveValue")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("BaselineDate")
                        .HasColumnType("date");

                    b.Property<decimal?>("BaselineValue")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("Date1")
                        .HasColumnType("date");

                    b.Property<DateTime?>("Date2")
                        .HasColumnType("date");

                    b.Property<string>("Frequency1")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Frequency2")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("MeasuringUnit")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Outcome")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("OutcomeBaseIndicator")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("PrimarySource")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("ProjectGoal")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.Property<string>("ResultBaseIndicator")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<decimal?>("TargetValue")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("LogFrame1stStepIndicatorId");

                    b.HasIndex("ProjectId")
                        .IsUnique();

                    b.ToTable("LogFrame1stStepIndicator");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame1stStepParticipant", b =>
                {
                    b.Property<int>("LogFrame1stStepParticipantId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<int>("LogFrame1stStepIndicatorId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectBeneficiaryTypeId")
                        .HasColumnType("int");

                    b.HasKey("LogFrame1stStepParticipantId");

                    b.HasIndex("LogFrame1stStepIndicatorId");

                    b.HasIndex("ProjectBeneficiaryTypeId");

                    b.ToTable("LogFrame1stStepParticipant");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame2ndStepCity", b =>
                {
                    b.Property<int>("LogFrame2ndStepCityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CityId")
                        .HasColumnType("int");

                    b.Property<int>("LogFrame2ndStepOutputId")
                        .HasColumnType("int");

                    b.HasKey("LogFrame2ndStepCityId");

                    b.HasIndex("CityId");

                    b.HasIndex("LogFrame2ndStepOutputId");

                    b.ToTable("LogFrame2ndStepCity");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame2ndStepOutput", b =>
                {
                    b.Property<int>("LogFrame2ndStepOutputId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal?>("AchieveValue")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("BaselineDate")
                        .HasColumnType("date");

                    b.Property<decimal?>("BaselineValue")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("Date1")
                        .HasColumnType("date");

                    b.Property<DateTime?>("Date2")
                        .HasColumnType("date");

                    b.Property<string>("Frequency1")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Frequency2")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("MeasuringUnit")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Output")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("OutputBaseIndicator")
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<string>("PrimarySource")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.Property<decimal?>("TargetValue")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("LogFrame2ndStepOutputId");

                    b.HasIndex("ProjectId")
                        .IsUnique();

                    b.ToTable("LogFrame2ndStepOutput");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame2ndStepParticipant", b =>
                {
                    b.Property<int>("LogFrame2ndStepParticipantId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<int>("LogFrame2ndStepOutputId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectBeneficiaryTypeId")
                        .HasColumnType("int");

                    b.HasKey("LogFrame2ndStepParticipantId");

                    b.HasIndex("LogFrame2ndStepOutputId");

                    b.HasIndex("ProjectBeneficiaryTypeId");

                    b.ToTable("LogFrame2ndStepParticipant");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame3rdStepActivity", b =>
                {
                    b.Property<int>("LogFrame3rdStepActivityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal?>("AchieveValue")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("BaselineDate")
                        .HasColumnType("date");

                    b.Property<decimal?>("BaselineValue")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal?>("Budget")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("CurrencyMeasuringUnit")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<DateTime?>("Date1")
                        .HasColumnType("date");

                    b.Property<DateTime?>("Date2")
                        .HasColumnType("date");

                    b.Property<decimal?>("Expenditure")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("Frequency1")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Frequency2")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Frequency3")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("MeasuringUnit")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("PrimarySource")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.Property<string>("ReasonOfDeviation")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("SummaryOrRemarks")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<decimal?>("TargetValue")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("LogFrame3rdStepActivityId");

                    b.HasIndex("ProjectId")
                        .IsUnique();

                    b.ToTable("LogFrame3rdStepActivity");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame3rdStepCity", b =>
                {
                    b.Property<int>("LogFrame3rdStepCityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CityId")
                        .HasColumnType("int");

                    b.Property<int>("LogFrame3rdStepActivityId")
                        .HasColumnType("int");

                    b.HasKey("LogFrame3rdStepCityId");

                    b.HasIndex("CityId");

                    b.HasIndex("LogFrame3rdStepActivityId");

                    b.ToTable("LogFrame3rdStepCity");
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame3rdStepParticipant", b =>
                {
                    b.Property<int>("LogFrame3rdStepParticipantId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<int>("LogFrame3rdStepActivityId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectBeneficiaryTypeId")
                        .HasColumnType("int");

                    b.HasKey("LogFrame3rdStepParticipantId");

                    b.HasIndex("LogFrame3rdStepActivityId");

                    b.HasIndex("ProjectBeneficiaryTypeId");

                    b.ToTable("LogFrame3rdStepParticipant");
                });

            modelBuilder.Entity("ProjectManagement.Data.Project", b =>
                {
                    b.Property<int>("ProjectId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DirectIndirectType")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("DonorType")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("date");

                    b.Property<string>("IndividualHouseholdType")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("KeyWord")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("Photo")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("ProjectName")
                        .IsRequired()
                        .HasColumnType("nvarchar(225)")
                        .HasMaxLength(225);

                    b.Property<int>("ProjectSectorId")
                        .HasColumnType("int");

                    b.Property<int?>("ProjectStatusId")
                        .HasColumnType("int");

                    b.Property<string>("ReportName")
                        .IsRequired()
                        .HasColumnType("nvarchar(225)")
                        .HasMaxLength(225);

                    b.Property<DateTime?>("StartDate")
                        .HasColumnType("date");

                    b.Property<DateTime?>("SubmissionDate")
                        .HasColumnType("date");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(225)")
                        .HasMaxLength(225);

                    b.Property<decimal>("TotalBudgetBdt")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal>("TotalBudgetUsd")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int>("TotalCount")
                        .HasColumnType("int");

                    b.Property<decimal>("TotalExpenditure")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("ProjectId");

                    b.HasIndex("ProjectSectorId");

                    b.HasIndex("ProjectStatusId");

                    b.ToTable("Project");
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectBeneficiary", b =>
                {
                    b.Property<int>("ProjectBeneficiaryId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Count")
                        .HasColumnType("int");

                    b.Property<int>("ProjectBeneficiaryTypeId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.HasKey("ProjectBeneficiaryId");

                    b.HasIndex("ProjectBeneficiaryTypeId");

                    b.HasIndex("ProjectId");

                    b.ToTable("ProjectBeneficiary");
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectBeneficiaryType", b =>
                {
                    b.Property<int>("ProjectBeneficiaryTypeId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("BeneficiaryType")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.HasKey("ProjectBeneficiaryTypeId");

                    b.ToTable("ProjectBeneficiaryType");
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectCity", b =>
                {
                    b.Property<int>("ProjectCityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CityId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.HasKey("ProjectCityId");

                    b.HasIndex("CityId");

                    b.HasIndex("ProjectId");

                    b.ToTable("ProjectCity");
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectDonor", b =>
                {
                    b.Property<int>("ProjectDonorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("DonorId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.HasKey("ProjectDonorId");

                    b.HasIndex("DonorId");

                    b.HasIndex("ProjectId");

                    b.ToTable("ProjectDonor");
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectReports", b =>
                {
                    b.Property<int>("ProjectReportsId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("FileUrl")
                        .IsRequired()
                        .HasColumnType("nvarchar(255)")
                        .HasMaxLength(255);

                    b.Property<int>("ProjectId")
                        .HasColumnType("int");

                    b.Property<int>("ReportTypeId")
                        .HasColumnType("int");

                    b.HasKey("ProjectReportsId");

                    b.HasIndex("ProjectId");

                    b.HasIndex("ReportTypeId");

                    b.ToTable("ProjectReports");
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectSector", b =>
                {
                    b.Property<int>("ProjectSectorId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Sector")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.HasKey("ProjectSectorId");

                    b.ToTable("ProjectSector");

                    b.HasData(
                        new
                        {
                            ProjectSectorId = 1,
                            Sector = "Access to Justice"
                        },
                        new
                        {
                            ProjectSectorId = 2,
                            Sector = "Activism along the Agricultural, Fishery, Dairy and Horticulture Value Chain"
                        },
                        new
                        {
                            ProjectSectorId = 3,
                            Sector = "Climate Emergency, Biodiversity and Disaster Management 1"
                        },
                        new
                        {
                            ProjectSectorId = 4,
                            Sector = "Climate Emergency, Biodiversity and Disaster Management 2"
                        },
                        new
                        {
                            ProjectSectorId = 5,
                            Sector = "Company as a social entrepreneurship"
                        },
                        new
                        {
                            ProjectSectorId = 6,
                            Sector = "Health and Nutrition"
                        },
                        new
                        {
                            ProjectSectorId = 7,
                            Sector = "Microfinance"
                        },
                        new
                        {
                            ProjectSectorId = 8,
                            Sector = "Youth Development (Education, skill, moral and cultural behavior)"
                        });
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectStatus", b =>
                {
                    b.Property<int>("ProjectStatusId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.HasKey("ProjectStatusId");

                    b.ToTable("ProjectStatus");
                });

            modelBuilder.Entity("ProjectManagement.Data.Registration", b =>
                {
                    b.Property<int>("RegistrationId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("CreatedOnUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getutcdate())");

                    b.Property<string>("DateofBirth")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<byte[]>("Image")
                        .HasColumnType("varbinary(max)");

                    b.Property<bool>("IsDeactivated")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("Phone")
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<int?>("ProjectSectorId")
                        .HasColumnType("int");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.HasKey("RegistrationId");

                    b.HasIndex("ProjectSectorId");

                    b.ToTable("Registration");

                    b.HasData(
                        new
                        {
                            RegistrationId = 1,
                            CreatedOnUtc = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsDeactivated = false,
                            Name = "Admin",
                            Type = "Admin",
                            UserName = "Admin"
                        });
                });

            modelBuilder.Entity("ProjectManagement.Data.ReportType", b =>
                {
                    b.Property<int>("ReportTypeId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ReportName")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.HasKey("ReportTypeId");

                    b.ToTable("ReportType");
                });

            modelBuilder.Entity("ProjectManagement.Data.State", b =>
                {
                    b.Property<int>("StateId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CountryId")
                        .HasColumnType("int");

                    b.Property<string>("StateName")
                        .IsRequired()
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.HasKey("StateId");

                    b.HasIndex("CountryId");

                    b.ToTable("State");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.City", b =>
                {
                    b.HasOne("ProjectManagement.Data.State", "State")
                        .WithMany("Cities")
                        .HasForeignKey("StateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame", b =>
                {
                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithOne("LogFrame")
                        .HasForeignKey("ProjectManagement.Data.LogFrame", "ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame1stStepCity", b =>
                {
                    b.HasOne("ProjectManagement.Data.City", "City")
                        .WithMany("LogFrame1stStepCities")
                        .HasForeignKey("CityId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.LogFrame1stStepIndicator", "LogFrame1stStepIndicator")
                        .WithMany("LogFrame1stStepCities")
                        .HasForeignKey("LogFrame1stStepIndicatorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame1stStepIndicator", b =>
                {
                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithOne("LogFrame1stStepIndicator")
                        .HasForeignKey("ProjectManagement.Data.LogFrame1stStepIndicator", "ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame1stStepParticipant", b =>
                {
                    b.HasOne("ProjectManagement.Data.LogFrame1stStepIndicator", "LogFrame1stStepIndicator")
                        .WithMany("LogFrame1stStepParticipants")
                        .HasForeignKey("LogFrame1stStepIndicatorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.ProjectBeneficiaryType", "ProjectBeneficiaryType")
                        .WithMany("LogFrame1stStepParticipants")
                        .HasForeignKey("ProjectBeneficiaryTypeId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame2ndStepCity", b =>
                {
                    b.HasOne("ProjectManagement.Data.City", "City")
                        .WithMany("LogFrame2ndStepCities")
                        .HasForeignKey("CityId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.LogFrame2ndStepOutput", "LogFrame2ndStepOutput")
                        .WithMany("LogFrame2ndStepCities")
                        .HasForeignKey("LogFrame2ndStepOutputId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame2ndStepOutput", b =>
                {
                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithOne("LogFrame2ndStepOutput")
                        .HasForeignKey("ProjectManagement.Data.LogFrame2ndStepOutput", "ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame2ndStepParticipant", b =>
                {
                    b.HasOne("ProjectManagement.Data.LogFrame2ndStepOutput", "LogFrame2ndStepOutput")
                        .WithMany("LogFrame2ndStepParticipants")
                        .HasForeignKey("LogFrame2ndStepOutputId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.ProjectBeneficiaryType", "ProjectBeneficiaryType")
                        .WithMany("LogFrame2ndStepParticipants")
                        .HasForeignKey("ProjectBeneficiaryTypeId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame3rdStepActivity", b =>
                {
                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithOne("LogFrame3rdStepActivity")
                        .HasForeignKey("ProjectManagement.Data.LogFrame3rdStepActivity", "ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame3rdStepCity", b =>
                {
                    b.HasOne("ProjectManagement.Data.City", "City")
                        .WithMany("LogFrame3rdStepCities")
                        .HasForeignKey("CityId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.LogFrame3rdStepActivity", "LogFrame3rdStepActivity")
                        .WithMany("LogFrame3rdStepCities")
                        .HasForeignKey("LogFrame3rdStepActivityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.LogFrame3rdStepParticipant", b =>
                {
                    b.HasOne("ProjectManagement.Data.LogFrame3rdStepActivity", "LogFrame3rdStepActivity")
                        .WithMany("LogFrame3rdStepParticipants")
                        .HasForeignKey("LogFrame3rdStepActivityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.ProjectBeneficiaryType", "ProjectBeneficiaryType")
                        .WithMany("LogFrame3rdStepParticipants")
                        .HasForeignKey("ProjectBeneficiaryTypeId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.Project", b =>
                {
                    b.HasOne("ProjectManagement.Data.ProjectSector", "ProjectSector")
                        .WithMany("Projects")
                        .HasForeignKey("ProjectSectorId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.ProjectStatus", "ProjectStatus")
                        .WithMany("Projects")
                        .HasForeignKey("ProjectStatusId")
                        .OnDelete(DeleteBehavior.NoAction);
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectBeneficiary", b =>
                {
                    b.HasOne("ProjectManagement.Data.ProjectBeneficiaryType", "ProjectBeneficiaryType")
                        .WithMany("ProjectBeneficiaries")
                        .HasForeignKey("ProjectBeneficiaryTypeId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithMany("ProjectBeneficiaries")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectCity", b =>
                {
                    b.HasOne("ProjectManagement.Data.City", "City")
                        .WithMany("ProjectCities")
                        .HasForeignKey("CityId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithMany("ProjectCities")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectDonor", b =>
                {
                    b.HasOne("ProjectManagement.Data.Donor", "Donor")
                        .WithMany("ProjectDonors")
                        .HasForeignKey("DonorId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithMany("ProjectDonors")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.ProjectReports", b =>
                {
                    b.HasOne("ProjectManagement.Data.Project", "Project")
                        .WithMany("ProjectReports")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagement.Data.ReportType", "ReportType")
                        .WithMany("ProjectReports")
                        .HasForeignKey("ReportTypeId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagement.Data.Registration", b =>
                {
                    b.HasOne("ProjectManagement.Data.ProjectSector", "ProjectSector")
                        .WithMany("Users")
                        .HasForeignKey("ProjectSectorId")
                        .OnDelete(DeleteBehavior.NoAction);
                });

            modelBuilder.Entity("ProjectManagement.Data.State", b =>
                {
                    b.HasOne("ProjectManagement.Data.Country", "Country")
                        .WithMany("States")
                        .HasForeignKey("CountryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
